...
namespace LegendsViewer.Legends.Events
{
    public class Gamble : WorldEvent
    {
        ...
        public override string Print(bool link = true, DwarfObject pov = null)
        {
            ...
            if (balance >= 5000)
            {
                eventString += " made a fortune gambling";
            }
            else if (balance >= 1000)
            {
                eventString += " did well gambling";
            }
            else if (balance <= -1000)
            {
                eventString += " did poorly gambling";
            }
            else if (balance <= -5000)
            {
                eventString += " lost a fortune gambling";
            }
            else
            {
                eventString += " breaks even gambling";
            }
            if (Site != null)
            {
                eventString += " in ";
